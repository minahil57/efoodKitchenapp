...
class OrderController extends GetxController implements GetxService{
    ...
    scrollController.addListener(() {
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent) {
        if (_offset < _pageSize) {
          print('api is call -----$_currentIndex');
          _offset++;
          if(_currentIndex == 0){
  ...
  Future<void> getOrderList(int offset, {bool reload = true}) async {
    ...
    if(response.statusCode == 200) {
      ...
      OrderModel _orderModel = OrderModel.fromJson(response.body);
      _orderModel.data!.forEach((order) {
        _orderList!.add(order);
      });
      _pageSize = _orderModel.lastPage!;
      _orderLength = _orderModel.total!;
  ...
  Future<void> orderStatusUpdate(int orderId, String orderStatus) async {
    ...
    if(response.statusCode == 200) {
      ...
      if(Get.width < 640){
        Get.to(HomeScreen(fromFilter: true));
      }
